            ...
            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                ...
                @if(sizeof($resultParty) > 0)   
                    ...
                    @foreach($resultParty as $retailers)
                         ...
                         @php
                            $resultRetailerIMS = DB::table('mts_visit_order') 
                    ...
                    <tr>
                        ...
                        <td><a href="{{ URL('/mts-order-process/'.$retailers->party_id.'/'.$customerID) }}"> Order </a></td>
                        @if($order==1 || $visit==1 && $non_visit==0)
                        <td></td>
                        <td></td>
                        @else
                        <td><a href="{{ URL('/mts-visit-order/'.$retailers->party_id.'/'.$customerID) }}"> Visit </a></td>
                        <td>
                            @if($order==0 && $visit==0 && $non_visit==1)
                            
                            @else
                            <a href="{{ URL('/mts-nonvisit/'.$retailers->party_id.'/'.$customerID) }}"> Non-visit </a>
                            @endif
                        </td>
                        @endif
