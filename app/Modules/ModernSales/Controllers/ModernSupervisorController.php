...
use App\Modules\ModernSales\Models\Supervisor; 
use DB;
use Auth;
use Session;

class ModernSupervisorController extends Controller
{ 
    public function define_supervisor()
    {
        ...
        $data = Supervisor::get();    
        return view('ModernSales::sales/supervisor/supervisor',compact('user_type','management','managers','executive','officer','selectedMenu','selectedSubMenu','pageTitle','data'));
    } 
    public function get_supervisor($id){
        $user_type = DB::table('tbl_user_type')->get();
        $management = DB::table('users')->where('user_type_id','5')->where('module_type','2')->get();
        $managers = DB::table('users')->where('user_type_id','6')->where('module_type','2')->get();
        $executive = DB::table('users')->where('user_type_id','3')->where('module_type','2')->get();
        $officer = DB::table('users')->where('user_type_id','7')->where('module_type','2')->get();
        $data = Supervisor::where('hierarchy_id',$id)->first(); 
        $supervisor = DB::table('users')->where('user_type_id',$data->supervisor_type)->where('module_type','2')->get(); 
        return view('ModernSales::sales/supervisor/edit',compact('user_type','management','managers','executive','officer','supervisor','data'));
    } 
    ...
    { 
        ...
        DB::table('mts_role_hierarchy')->insert([            
        ...
        return redirect('/mst-supervisor')->with('success','Supervisor add sucessfully.');
    }
    ...
    {  
        ...
        $data->update(); 
        return redirect('/mst-supervisor')->with('success','Supervisor update sucessfully.');
    }

    public function supervisor_delete($id){        
        DB::table('mts_role_hierarchy')->where('hierarchy_id',$id)->delete();
        return redirect('/mst-supervisor')->with('success','Supervisor delete sucessfully.');
    }
