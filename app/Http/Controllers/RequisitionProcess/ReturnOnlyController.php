...
class ReturnOnlyController extends Controller
{
    ...
    **/
	...
	public function returnproduct()
    {
        ...
        $checkRoutes = DB::table('tbl_return_only_distributor')->select('route_id')
        ...
        {
            $checkRoutes = DB::table('tbl_visit_return_only_distributor')->select('routeid')
        ...
        return view('sales/requisitionProcess/returnManage', compact('selectedMenu','pageTitle','routeResult','resultRetailer','routeID'));
    }
	...
	public function return_retailer(Request $request)
    {
        ...
        return view('sales/requisitionProcess/retailers', compact('resultRetailer','routeID'));
    }
	...
	public function return_process($retailderid,$routeid)
    {
        ...
        $resultCart     = DB::table('tbl_return_only_distributor')
                        ->where('return_only_order_type','Ordered')                        
                        ->where('fo_id',Auth::user()->id)                        
                        ->where('retailer_id',$retailderid)                        
                        ->first();

        return view('sales/requisitionProcess/categoryWiseReturn', compact('selectedMenu','pageTitle','resultRetailer','resultCategory','retailderid','routeid','pointID','distributorID','resultCart'));
    }
	...
	public function return_products(Request $request)
    {
        ...
        return view('sales/requisitionProcess/returnProductsList', compact('resultProduct','categoryID','pcategory'));
    }
	...
	public function get_product(Request $request)
    {
        ...
        return view('Sales/requisitionProcess/getProduct' , compact('product','serial'));
	...
	public function return_add_to_cart_products(Request $request)
    {
		...
		$autoOrder = DB::table('tbl_return_only_distributor')->select('return_only_auto_order_no')->orderBy('return_only_order_id','DESC')->first();
	   ...
	   // for total return value		
        ...
        {
            DB::table('tbl_return_only_distributor')->insert(
                [
            ...
            $lastOrderId = DB::table('tbl_return_only_distributor')->latest('return_only_order_id')->first();
            ...
            for($m=0;$m<$countReturnRows;$m++)
            {
                if($request->get('return_qty')[$m]!='')
                {
                    ...
                    DB::table('tbl_return_details_only_distributor')->insert(
                        [
            ...
            return Redirect::to('/fo/return-only-process/'.$request->get('retailer_id').'/'.$request->get('route_id'))->with('success', 'Successfully Added Add To Cart.');
        }
        elseif(sizeof($resultCart) > 0)
        {
            ...
            for($m=0;$m<$countReturnRows;$m++)
            {
                if($request->get('return_qty')[$m]!='')
                {
                    $checkItemsExiting = DB::table('tbl_return_details_only_distributor')
                                    ->select('tbl_return_details_only_distributor.*','tbl_return_only_distributor.return_only_order_id','tbl_return_only_distributor.return_only_order_type')
                                    ->join('tbl_return_only_distributor', 'tbl_return_only_distributor.return_only_order_id', '=', 'tbl_return_details_only_distributor.return_only_order_id')
                                    ->where('tbl_return_only_distributor.return_only_order_type', 'Ordered')
                                    ->where('tbl_return_only_distributor.return_only_order_id', $lastOrderId)
                                    ->where('tbl_return_details_only_distributor.return_only_product_id',$request->get('produuct_id')[$m])
                                    ->first();
            ...
            return Redirect::to('/fo/return-only-process/'.$request->get('retailer_id').'/'.$request->get('route_id'))->with('success', 'Successfully Updated Add To Cart.');
        } 
    }
    ...
    {
        $selectedMenu   = 'Requisition';           // Required Variable
        $pageTitle      = 'Return Bucket';              // Page Slug Title
		...
		$resultCartPro  = DB::select("select `tbl_return_details_only_distributor`.*, `c1`.`id` as `ret_catid`, `c1`.`name` as `ret_catname`, 
								`p1`.`id`, `p1`.`name` as `ret_cproname`, 
							
								`tbl_return_only_distributor`.`return_only_order_id`, `tbl_return_only_distributor`.`fo_id`, 
								`tbl_return_only_distributor`.`return_only_order_type`, `tbl_return_only_distributor`.`retailer_id` 
								from `tbl_return_details_only_distributor` inner join tbl_product_category as c1 on `c1`.`id` = `tbl_return_details_only_distributor`.`return_only_cat_id` 
							
							inner join `tbl_return_only_distributor` on `tbl_return_only_distributor`.`return_only_order_id` = `tbl_return_details_only_distributor`.`return_only_order_id` 
							inner join tbl_product as p1 on `p1`.`id` = `tbl_return_details_only_distributor`.`return_only_product_id` 
						
							where `tbl_return_only_distributor`.`return_only_order_type` = 'Ordered' 
							and `tbl_return_only_distributor`.`fo_id` = '".Auth::user()->id."' and `tbl_return_only_distributor`.`retailer_id` = '".$retailderid."'");
        ...
        $resultInvoice  = DB::table('tbl_return_only_distributor')->select('return_only_order_id','return_only_order_type','fo_id','retailer_id','return_only_order_no','return_only_auto_order_no')
                        ->where('return_only_order_type','Ordered')                        
                        ->where('fo_id',Auth::user()->id)                        
                        ->where('retailer_id',$retailderid)
                        ->first();

        return view('sales/requisitionProcess/returnBucket', compact('selectedMenu','pageTitle','pointid','retailderid','routeid','distributorID','resultRetailer','resultCartPro','resultInvoice'));
    }
    ...
    public function return_items_edit(Request $request)
    {
        ...
        $resultPro  = DB::table('tbl_return_details_only_distributor')
                        
						->select('tbl_return_details_only_distributor.*',
						'tbl_return_details_only_distributor.p_unit_price','p1.name as ret_proName')
                        
						->join('tbl_product as p1', 'p1.id', '=', 'tbl_return_details_only_distributor.return_only_product_id')
						//->join('tbl_product as p2', 'p2.id', '=', 'tbl_return_details_only_distributor.change_product_id')
                        
						->where('tbl_return_details_only_distributor.return_only_order_det_id', $request->get('id'))
                        ->first();						

        return view('sales/requisitionProcess/editReturnItems', compact('resultPro','pointid','retailderid','routeid','catid'));
    }
	...
	public function return_items_edit_submit(Request $request)
    {
			...
			DB::table('tbl_return_details_only_distributor')->where('return_only_order_det_id',$request->get('id'))->update(
                [
            ...
            $totalQty = DB::table('tbl_return_details_only_distributor')
            ...
            DB::table('tbl_return_only_distributor')->where('return_only_order_id', $totalQty->return_only_order_id)
                ->where('fo_id', Auth::user()->id)
                ->where('return_only_order_type', 'Ordered')
                ->where('global_company_id', Auth::user()->global_company_id)->update(
                [
    ...
    public function return_items_delete(Request $request)
    {
        ...
        $itemsDelete = DB::table('tbl_return_details_only_distributor')->where('return_only_order_det_id',$id)->delete();
        ...
        //dd($totalQty);
         ...
         DB::table('tbl_return_only_distributor')->where('return_only_order_id', $orderID)
                ->where('fo_id', Auth::user()->id)
                ->where('return_only_order_type', 'Ordered')
                ->where('global_company_id', Auth::user()->global_company_id)->update(
                [
    ...
    public function ssg_confirm_return(Request $request, $orderpid,$orderid,$retailderid,$routeid,$pointid,$distributorID)
    {
        ...
        DB::table('tbl_visit_return_only_distributor')->insert(
            [
        ...
        DB::table('tbl_return_only_distributor')->where('return_only_order_id', $orderpid)->where('fo_id', Auth::user()->id)->update(
            [
                'return_only_order_type'             => 'Confirmed'
            ]
        );

        return Redirect::to('/fo/return-only-product')->with('success', 'Successfully Confirmed Return.');
    }
    ...
    public function ssg_delete_return(Request $request)
    {
        ...
        $lastOrderId = DB::table('tbl_return_only_distributor')->select('return_only_order_type','fo_id','return_only_order_id','return_only_order_no')
                    ->where('return_only_order_id',$orderid)->first();

        $orderID     = $orderid;
       
        DB::table('tbl_visit_return_only_distributor')->where('order_no', $lastOrderId->return_only_order_no)->delete();

        DB::table('tbl_return_only_distributor')->where('return_only_order_id', $orderID)->delete();
