                        ...
                        @if(sizeof($resultOrderList) > 0)   
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;">
                                    <form action="{{ URL('/sys/invoice-delete') }}" method="post">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">

                                        <input type="hidden" name="orderid" value="{{ $orders->order_id }}">
                                        
                                    <input type="submit" class="btn btn-block bg-pink waves-effect" name="delete" id="delete" value="Delete" onclick="return confirm('Are you sure you want to delete this invoice?');">
                                   
                                    </form>

                                    <a href="{{ URL('sys/invoice-details/'.$orders->order_id.'/'.$orders->fo_id) }}" target="_blank" title="Click To View Invoice Details">
