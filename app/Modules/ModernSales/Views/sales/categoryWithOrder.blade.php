...
@section('content')
    <section class="content">
        <div class="container-fluid">
            ...
            <form action="{{ URL('/mts-add-to-cart') }}" method="POST">
                {{ csrf_field() }}  
                <input type="hidden" id="customer_id" name="customer_id" value="{{ $customer_id }}">
                <input type="hidden" id="retailer_id" name="retailer_id" value="{{ $partyid }}">
                <input type="hidden" id="cat_id" name="cat_id" value="">
                <input type="hidden" id="order_id" name="order_id" value="@if(sizeof($resultCart)>0){{$resultCart->order_id}}@else{{0}}@endif">
