                        ...
                        <div class="col-sm-5">
                            <select id="fos" class="form-control show-tick" data-live-search="true">
                                ...
                                @foreach($resultFO as $fos)
                                    <option value="{{ $fos->user_id }}">{{ $fos->first_name.''.$fos->middle_name.''.$fos->last_name }}</option>
                                @endforeach                                                    
                            </select>
                        </div>

                        <div class="col-sm-2">
                            <button type="button" class="btn bg-pink btn-block btn-lg waves-effect" onclick="allDeliveryreturn()">Search</button>
                        </div>
                    </div>                                  
                </div>
            </div>
                            ...
                            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                @if(sizeof($resultOrderList) > 0)   
                                    ...
                                    <tr>
                                        <th>{{ $serial }}</th>
                                        <th>
                                            <a href="{{ URL('/eshop-return-delivery-edit/'.$orders->return_id.'/'.$orders->fo_id) }}" title="Confirm Delivery" target="_blank">
                                                {{ $orders->return_no }}
                                            </a>
                                        </th>
                                        <th>{{ $orders->return_date }}</th>
