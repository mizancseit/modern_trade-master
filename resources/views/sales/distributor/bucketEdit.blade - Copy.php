                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <th style="text-align: right;">
                                                    <input type="text" name="qty[]" id="qty{{ $serial }}" value="@if($itemsPro->delivered_qty==''){{$itemsPro->order_qty}}@else{{$itemsPro->delivered_qty}} @endif" class="form-control" style="width: 100px;" onkeyup="qty({{ $serial }},{{ $itemsPro->order_qty }})" min="1" maxlength="8" autocomplete="off">
                                                </th>
                                                ...
                                                <th style="text-align: right;">
                                                    <input type="text" name="replaceDelivery[]" id="replaceDelivery{{ $serial }}" value="@if($itemsPro->wastage_qty==''){{0}}@else{{ $itemsPro->wastage_qty }} @endif" class="form-control" style="width: 100px;" autocomplete="off">
                                                </th>
                                                </tr>
                                                @php
                                                $serial ++;
                                                ...
                                                @php                                             
                                                    ...
                                                    ->select('fq.type','fq.status','fq.order_id','fq.auto_order_no','fq.catid','fq.product_id','fq.total_free_qty','tbl_product.id','tbl_product.name AS proname','fq.free_id')
                                            ...
                                            <tr>
                                                <th colspan="3" align="left" id="memoCommission">MEMO COMMISSION : {{ number_format($com,2) }}%</th>
                                                <th>&nbsp;</th>
                                                <th>&nbsp;</th>
                                                <th>&nbsp;</th>
                                                <th>&nbsp;</th>
                                        ...
                                        @if(sizeof($orderTypePartial)>0 && $orderTypePartial->order_det_status=='Closed')
                                            ...
                                            <div class="col-sm-1">                                                
                                                @foreach($resultBundleOffers as $offers)
                                                    <input name="offer_type" type="radio" id="radio_7{{ $offers->iId }}" class="radio-col-red" value="{{ $offers->iId }}" data-toggle="modal" @if($bundleOffersCheck >0) checked="" @endif onclick="showBundleProduct('{{ $offers->iId }}','2')">
                                                    <label for="radio_7{{ $offers->iId }}"> Bundle </label>
                                                @endforeach                                        
                                            </div>
                                            @endif
                                         @endif
