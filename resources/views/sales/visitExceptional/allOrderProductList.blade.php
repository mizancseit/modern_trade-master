                        ...
                        <tbody>
                            @if(sizeof($resultProduct) > 0)
                                ...
                                @foreach($resultProduct as $products)
                                 ...
                                 @php
                                    $pOrderQty  = DB::table('tbl_order_details')
                                            ->select('order_qty','p_total_price')
                                            ->where('order_id',$lastOrderid)
                                            ->where('tbl_order_details.order_qty','!=','')
                                            ->where('product_id',$products->id)
                                            ->first();

                                    $pWastageQty  = DB::table('tbl_order_details')
                                            ->select('wastage_qty')
                                            ->where('order_id',$lastOrderid)
                                            ->where('tbl_order_details.wastage_qty','!=','')
                                            ->where('product_id',$products->id)
                                            ->first();
                                   ...
                                   $stockResult = DB::select("SELECT product_id,(sum(if(inventory_type=1,product_qty,0))- sum(if(inventory_type=2,product_qty,0))) as stock_qty FROM depot_inventory 
                                        WHERE point_id =$pointID AND product_id=$products->id");

                                   $demandQty  = DB::table('tbl_order_details')
                                            ->join('tbl_order', 'tbl_order_details.order_id', '=', 'tbl_order.order_id')
                                            ->where('tbl_order.point_id',$pointID)
                                            ->where('tbl_order_details.product_id',$products->id)
                                            ->where('tbl_order_details.order_det_type','Confirmed')
                                            ->sum('tbl_order_details.order_qty');
                                ...
                                <tr>
                                    ...
                                    <th>{{ $products->name }}</th>
                                     @if(sizeof($pOrderQty) > 0)
                                    <th><input type="number" class="form-control" id="qty{{$serial}}" name="qty[]" maxlength="8" pattern="[1-9]" min="1" value="{{$pOrderQty->order_qty}}" style="width: 80px;" onkeyup="addQty({{$serial}})" onmouseout="totalsFO();" readonly="">
                                    </th>
                                    <th><input type="number" class="form-control" id="value{{$serial}}" name="value[]" maxlength="8" value="{{$pOrderQty->p_total_price}}" style="width: 100px;" readonly=""></th>
                                    @else
                                    ...
                                    @endif
                                     @if(sizeof($pWastageQty) > 0)
                                    <th><input type="number" class="form-control" id="wastageQty{{$serial}}" name="wastageQty[]" maxlength="8" value="{{$pWastageQty->wastage_qty}}" style="width: 80px;" onkeyup="totalsWastagesFO();addWastage({{$serial}})"></th>
                                     @else 
                                     <th><input type="number" class="form-control" id="wastageQty{{$serial}}" name="wastageQty[]" maxlength="8" value="" style="width: 80px;" onkeyup="totalsWastagesFO();addWastage({{$serial}})"></th>
                                      @endif
                                     <th>@if(sizeof($stockResult[0]->stock_qty) > 0){{$stockResult[0]->stock_qty}} @else {{'0'}} @endif</th>
                                     <th>@if(sizeof($demandQty) > 0){{$demandQty}} @else {{'0'}} @endif</th> 
