...
class ReturnController extends Controller
{
    ...
    **/
	...
	public function return_products(Request $request)
    {
        ...
        return view('sales.return.returnProductsList', compact('resultProduct','categoryID','pcategory'));
    }
     ...
     public function return_bucket($pointid,$routeid,$retailderid)
    {
        ...
        */
        ...
        $resultCartPro  = DB::select(" select `tbl_return_details`.*, `c1`.`id` as `ret_catid`, `c1`.`name` as `ret_catname`,
                            ...
                            inner join tbl_product as p1 on `p1`.`id` = `tbl_return_details`.`return_product_id` 
                            inner join tbl_product as p2 on `p2`.`id` = `tbl_return_details`.`change_product_id` 
                            where `tbl_return`.`return_order_type` = 'Ordered' 
    ...
    public function ssg_confirm_return(Request $request, $orderpid,$orderid,$retailderid,$routeid,$pointid,$distributorID)
    {
        ...
        DB::table('tbl_return')->where('return_order_id', $orderpid)->where('fo_id', Auth::user()->id)->update(
            [
