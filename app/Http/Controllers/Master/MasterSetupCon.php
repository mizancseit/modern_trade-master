...
use App\Models\Sales\GlobalCompanyModel;
...
public function company_delete(Request $request)
	...
	//GLOBAL COMPANY SET UP START
	
	public function global_company_setup()
	{
	
		$selectedMenu   = 'globalcompany';         		// Required Variable
        $pageTitle      = 'Global Company List';         // Required Variable
      

		$GlobalCompanyList = DB::table('tbl_global_company')->where('is_active',0)->get();
                        
        
		return view('Master.global_company_setup')->with('GlobalCompanyList',$GlobalCompanyList)
	...
	public function global_company_process(Request $req)
	{
		if ($req->isMethod('post')) 
		{
			$selectedMenu   = 'globalcompany';         // Required Variable
			...
			$global_company_name		=	$req->input('global_company_name');
			$global_company_owner		=	$req->input('global_company_owner');
			$global_company_email		=	$req->input('global_company_email');
			$global_company_phone		=	$req->input('global_company_phone');
			$global_company_address		=	$req->input('global_company_address');
			
			
			$objGlobalCompany =  new GlobalCompanyModel();
			
			$objGlobalCompany->global_company_name = 	$global_company_name;
			$objGlobalCompany->global_company_owner = 	$global_company_owner;
			$objGlobalCompany->global_company_email = 	$global_company_email;
			$objGlobalCompany->global_company_phone = 	$global_company_phone;
			$objGlobalCompany->global_company_address = $global_company_address;
			$objGlobalCompany->updated_by = Auth::user()->id;;
			$objGlobalCompany->updated_at = date('Y-m-d');
			
			$objGlobalCompany->save();
			
			return Redirect::to('/globalCompany')->with('success', 'Successfully global company created.');
			   
		}
	}
	
	
	public function global_company_edit(Request $req)
	{
		$selectedMenu   = 'globalcompany';         		// Required Variable
        $pageTitle      = 'Global Company Edit';         // Required Variable
		
		$slID	=	$req->get('id');
		
		$globalCompanyList	=	DB::table('tbl_global_company')->where('global_company_id',$slID)
															   ->first();
								
		return view('Master.global_company_edit', compact('pageTitle','selectedMenu','globalCompanyList'));
	}
	
	
	
	public function global_company_edit_process(Request $req)
	{
		...
		if ($req->isMethod('post')) 
		{
			...
			$company= DB::table('tbl_global_company')->where('global_company_id',$req->get('id'))->update(
            [
                'global_company_name'         => $global_company_name,
                'global_company_owner'             => $global_company_owner,
                'global_company_email'          => $global_company_email,
                'global_company_phone'           => $global_company_phone,
                'global_company_address'              => $global_company_address,
			...
			/*
			$objGlobalCompany = GlobalCompanyModel::find($req->input('id'));
			...
			*/
			...
			return Redirect::to('/globalCompany')->with('success', 'Successfully Updated  Global Company.');
		}

	}
	

	public function global_company_delete(Request $request)
    {
        $id  = $request->get('id');
    	
		$user= DB::table('tbl_global_company')->where('global_company_id',$request->get('id'))->update(
          [
            'is_active'      => 1,
            'updated_by'     => Auth::user()->id,
            'updated_at'     => date('Y-m-d')
      ...
      return Redirect::to('/globalCompany')->with('success', 'Successfully deleted Global Company.');
      
    }

	//GLOBAL COMPANY SET UP END
...
public function territory_setup(Request $req)
{
	...
	$pageTitle      = 'Territory List';       // Required Variable
        ...
        $territory=DB::table('tbl_territory')
                        ...
                        ->leftjoin('tbl_business_type', 'tbl_business_type.business_type_id', '=', 'tbl_territory.business_type_id')
                        ->get();
	...
	public function distributor_setup(Request $req)
	{
	  ...
	  $route_id=$req->get('id');
		...
		if($route_id)
		{
			...
			LEFT JOIN tbl_point on tbl_user_business_scope.point_id=tbl_point.point_id
			LEFT JOIN tbl_division on tbl_user_business_scope.division_id=tbl_division.div_id
...
$proCategory=DB::select("select tbl_product_category.*,tbl_global_company.global_company_name as cname
from tbl_product_category
LEFT JOIN tbl_global_company ON tbl_product_category.company_id=tbl_global_company.global_company_id
...
else 
{
  ...
  $tbl_company=DB::select('SELECT  *  FROM tbl_global_company');
...
///////////////////////////////////////// Reject Reseon Begin  ///////////////////////////////////////////////////////
  ...
  public function retailer_setup(Request $req)
  {
    ...
    if($retailer_id)
    {
      ...
      $retailer_details=DB::select("SELECT  d.div_name, p.point_name pname, rt.rname, ret.*
      FROM tbl_retailer ret
      JOIN tbl_division d ON d.div_id = ret.division  
      LEFT JOIN tbl_point p ON p.point_id = ret.point_id
      ...
      $point_list   = DB::table('tbl_point')->get();
      $division     = DB::select('SELECT * from tbl_division');
      ...
      $retailer_details=DB::select('SELECT  d.div_name, c.global_company_name as company_name, p.point_name, rt.rname, ret.*
      FROM tbl_retailer ret
      JOIN tbl_division d ON d.div_id = ret.division  
      LEFT JOIN tbl_point p ON p.point_id = ret.point_id
      LEFT JOIN tbl_global_company c ON c.global_company_id = ret.global_company_id
      JOIN tbl_route rt ON ret.rid = rt.route_id order by ret.retailer_id desc limit 1000');
      ...
      return view('Master.retailer_setup')->with('pageTitle',$pageTitle)->with('selectedMenu',$selectedMenu)
                              ...
                              ->with('companyList',$companyList)
                              ->with('point_list',$point_list)->with('retailer_details',$retailer_details);
    }
  ...
  public function retailer_process(Request $req)
  {
    ...
    if ($req->isMethod('post')) 
    {
      ...
      if($req->input('id'))
      {
        ...
        $obJRetailerModel->point_id        	= $point_id;
        ...
        $obJRetailerModel->serial         	= $serial;
  ...
  // Zubair Bhai Ends Retailer//
    ...
    public function define_supervisor()
    {
        ...
        $supervisorList  = DB::table('tbl_supervisor')
        				->select('tbl_supervisor.iId','users.id','tbl_user_type.user_type_id','users.display_name','tbl_user_type.user_type','tbl_supervisor.iPointId','tbl_point.point_name','tbl_supervisor.iDivId','tbl_division.div_name','tbl_supervisor.iSuperId','users.is_active')
                        ->join('users', 'users.id', '=', 'tbl_supervisor.iSuperId')
                        ->join('tbl_user_type', 'tbl_user_type.user_type_id', '=', 'tbl_supervisor.iSuperType')
                        ->join('tbl_division', 'tbl_division.div_id', '=', 'tbl_supervisor.iDivId')
                        ->join('tbl_point', 'tbl_point.point_id', '=', 'tbl_supervisor.iPointId')
                        ->where('users.is_active', '=', '0')
                        ->orderBy('tbl_supervisor.iId','ASC')                    
                        ->get();

         return view('Master.user_supervisor' , compact('selectedMenu','selectedSubMenu','pageTitle','user_type','supervisorList','division'));  
    ...
    public function get_user_list(Request $request)
    {
          ...
          $point=DB::table('tbl_point')
                     ->where('point_division',$id)
                     ->get();


        return view('Master.getUsers' , compact('users','point'));
    ...
    public function get_supervisor_list(Request $request)
    {
        ...
        return view('Master.getSupervisor' , compact('users'));
    ...
    public function supervisor_save(Request $request)
    {
    	...
    	$countRows = count($request->get('point'));        

        for($m=0;$m<$countRows;$m++)
        {
        	if($request->get('point')[$m]!='')
            {

	           DB::table('tbl_supervisor')->insert(
	              [
	              'iPointId'            => $request->get('point')[$m],
	              'iDivId'              => $request->get('division'),
	              'iSuperId'            => $request->get('supervisor'),
	              'iSuperType'          => $request->get('supervisorType'),
        ...
        return redirect('/userSupervisor')->with('success','Supervisor add sucessfully.');
        
    }

    public function supervisor_delete(Request $request){

    	
    	 DB::table('tbl_supervisor')->where('iId',$request->get('id'))->delete();

    	 return redirect('/userSupervisor')->with('success','Supervisor delete sucessfully.');
