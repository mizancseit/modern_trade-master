...
class VisitControllerException extends Controller
{
    /**
    *
    ...
    **/
    ...
    public function ssg_visit()
    {
        ...
        $routeResult = DB::table('tbl_user_business_scope')
                        ->select('tbl_user_business_scope.global_company_id','tbl_user_business_scope.point_id','tbl_user_business_scope.user_id','tbl_user_business_scope.territory_id','tbl_route.point_id','tbl_route.rname','tbl_route.route_id')
        ...
        return view('sales/visitExceptional/visitManage', compact('selectedMenu','pageTitle','routeResult','resultRetailer','routeID'));
    }
    ...
    public function ssg_retailer(Request $request)
    {
        ...
        return view('sales/visitExceptional/retailers', compact('resultRetailer','routeID'));
    }

    public function ssg_order_process($retailderid,$routeid,$offerType)
    {
        $selectedMenu   = 'Visit';             // Required Variable
        ...
        $orderBucketSum = DB::table('tbl_order_details')
                        ->select('order_id','partial_order_id', DB::raw('SUM(p_total_price) AS grand_total_value'))
                        ->where('order_id', $orderid)
                        ->groupBy('partial_order_id')
                        ->orderBy('order_det_id')
                        ->get(); 
        ...
        return view('sales/visitExceptional/categoryWithOrder', 
        compact('selectedMenu','pageTitle','resultRetailer','resultCategory','retailderid','routeid','pointID','distributorID','resultCart','orderBucketSum'));
    }
    ...
    public function ssg_order_category_products(Request $request)
    {
        ...
        $offerTypeExc   = $request->get('offerTypeExc');
        ...
        $lastOrderid     = DB::table('tbl_order')
                        ->select('tbl_order.offer_type','tbl_order.order_id','tbl_order.order_type','tbl_order.retailer_id','tbl_order.fo_id','tbl_order_details.cat_id')
                        ...
                        ->where('tbl_order.offer_type',$offerTypeExc)
       ...
       //dd($retailerID);
        ...
        return view('sales/visitExceptional/allOrderProductList', compact('resultProduct','categoryID','pointID','retailerID','lastOrderid','offerTypeCategory'));
    }
    ...
    public function ssg_category_products(Request $request)
    {
       ...
       //dd($retailerID);
        ...
        return view('sales/visitExceptional/allProductList', compact('resultProduct','categoryID','retailerID','lastOrderid','offerTypeCategory'));
    }
    ...
    public function ssg_order_manage_category_products(Request $request)
    {
       ...
       //dd($retailerID);
        ...
        return view('sales/visitExceptional/allOrderProductList', compact('resultProduct','categoryID','retailerID','lastOrderid','offerTypeCategory','pointID'));
    }
    ...
    public function ssg_add_to_cart_products(Request $request)
    {
        ...
        // check new or exiting invoice
        $resultCart     = DB::table('tbl_order')                        
                        ->where('fo_id',Auth::user()->id)                        
                        ->where('retailer_id',$retailerID)
                        ->where('offer_type',$request->get('offerTypeExc'))
                        ->orderBy('order_id','DESC')                         
                        ->first(); 

        $lastOrderId =0;
        if(sizeof($resultCart)== 0 OR (sizeof($resultCart) >= 0  && $resultCart->order_status == 'Closed') )
        { 
            ...
            DB::table('tbl_order')->insert(
                [
                    ...
                    'order_status'          => 'Open',
                    'order_type'            => 'Ordered',
                    'offer_type'            => $request->get('offerTypeExc'),
            ...
            return Redirect::back()->with('success', 'Successfully Added Add To Cart.');
        }
        
        // if order already placed
        elseif(sizeof($resultCart) > 0 && $resultCart->order_status != 'Closed')
        {
            $lastOrderId = $resultCart->order_id;

            $pertialPart    = DB::table('tbl_order_details')                        
                        ->where('order_id',$lastOrderId)                        
                        ->where('order_det_type','=','Ordered')
                        ->orderBy('order_det_id','DESC')                         
                        ->first();
            ...
            if( $resultCart->order_count>=1 && sizeof($pertialPart)==0 )
            {
            ...
            $oldGrandTotal  = $resultCart->total_value;
           ...
           if( $resultCart->order_count>=1 && sizeof($pertialPart)==0)
            ...
            return Redirect::back()->with('failed', 'Maximum Order Exceed.');

            //return Redirect::to('/order-process/'.$request->get('retailer_id').'/'.$request->get('route_id'))->with('failed', 'Maximum Order Exceed.');
    ...
    public function ssg_bucket($pointid,$routeid,$retailderid,$partial_id,$offerType)
    {
        ...
        $resultRetailer = DB::table('tbl_retailer')
         ...
         $resultCartPro  = DB::table('tbl_order_details')
                        ...
                        'tbl_order.order_no','tbl_order.offer_type','tbl_order.retailer_id','tbl_special_commission.catid AS valuecatid',
                        'tbl_special_commission.commission','tbl_special_commission.total_free_value','tbl_order_details.ordered_date','tbl_order_details.order_det_type')
                        ...
                        ->where('tbl_order.offer_type',$offerType)
                        ->where('tbl_order_details.partial_order_id',$partial_id)
                        ->groupBy('tbl_order_details.cat_id')                        
                        ->get();
                      
        
        $resultInvoice  = DB::table('tbl_order')->select('offer_type','order_id','order_type','fo_id','retailer_id','order_no','auto_order_no','order_count')
                        ->where('order_status', '<>', 'Closed')                        
                        ->where('fo_id',Auth::user()->id)                        
                        ->where('offer_type',$offerType)           
                        ->where('retailer_id',$retailderid)
    ...
    //dd($resultBundleOffers);
        ...
        return view('sales/visitExceptional/bucket', compact('selectedMenu','pageTitle','pointid','retailderid','routeid','distributorID','resultRetailer','resultCartPro',
        'resultInvoice','resultBundleOffers','resultBundleOffersGift','specialOffers','commissionWiseItem','specialValueWise','partial_id','pointValueWise'));
    }
    
    
    public function ssg_bucket_offer($pointid,$routeid,$retailderid,$partialOrder,$offerType)
    {
       ...
       //dd($resultCartPro); exit;
        ...
        $resultInvoice  = DB::table('tbl_order')->select('order_id','order_type','fo_id','retailer_id','order_no','auto_order_no','grand_total_value')
                        ->where('order_status','Close_Req')                        
                        ->where('fo_id',Auth::user()->id)                        
                        ->where('retailer_id',$retailderid)
                        ->where('offer_type',$offerType)
        ...
        $checkRegularAvailableItems = DB::table('tbl_order_free_qty')
                    ->where('order_id', $oid)
                    ->whereNull('status')  //added items
                    ->count();

        $checkRegularAdded = DB::table('tbl_order_free_qty')
                    ->where('order_id', $oid)
                    ->where('status', 0)  //added items
                    ->count();
     ...
     $bundleCategoriesEx  = DB::table('tbl_bundle_except_category')
        ...
        //dd($resultBundleAvailableItems);

        return view('sales/visitExceptional/bucket_offer', compact('selectedMenu','pageTitle','pointid','retailderid','routeid','distributorID','resultRetailer','resultCartPro','resultInvoice','resultBundleOffers','resultBundleOffersGift','specialOffers','commissionWiseItem','specialValueWise','prevoiusBalanceCommission','pointValueWise','checkExclusiveAvailableItems','checkExclusiveAdded','checkRegularAvailableItems','checkRegularAdded','resultBundleAvailableItems','resultBundleAddedItems'));
    }
    ...
    public function ssg_bundle_gifts_added(Request $request)
    {
        ...
        //             $checkOffer = DB::table('tbl_order_gift')
        //                     ->where('global_company_id',Auth::user()->global_company_id)
        //                     ->where('orderid',$request->get('orderid'))
        //                     ->where('offerid',$request->get('offerid'))
        //                     ->where('retailer_id',$request->get('retailderid'))
     ...
     public function ssg_confirm_order(Request $request)
    {
        ...
        DB::table('ims_tbl_visit_order')->insert(
            [
                ...
                'offer_type'            => $request->get('offerType'), 
                'nonvisitedremarks'     => '',
        ...
        if($request->offer_confirm == 'NO')
        {
            DB::table('tbl_order_details')
            ->where('order_id', $request->get('orderid'))
            ->where('partial_order_id', $request->get('partial_id'))
            ->update(
                [
        ...
        return Redirect::to('/visit-exception')->with('success', 'Successfully Confirmed Order');
    }

    
    public function ssg_order_closed($order_id,$partialOrder,$offerType)
    {
       DB::table('tbl_order_details')
        ...
        return Redirect::to('/bucket-offer-exception'. '/' . $OrdData->point_id . '/' . $OrdData->route_id . '/' . $OrdData->retailer_id. '/' . $partialOrder. '/' . $offerType)->with('success', 'Successfully Initiate Close Request');
    ...
    public function ssg_order_manage_closed($order_id,$partialOrder,$offerType)
    {
        ...
        DB::table('tbl_order')->where('order_id', $order_id)
        ->where('offer_type', $offerType)
        ->where('fo_id', Auth::user()->id)->update(
            [
                'order_status'  => 'Closed',
                'order_type'    => 'Confirmed',
                'closing_date'  => date('Y-m-d')
        ...
        return Redirect::to('/bucket-edit-exception'. '/' . $order_id . '/'. $OrdData->point_id . '/' . $OrdData->route_id . '/' . $OrdData->retailer_id. '/' . $partialOrder. '/' . $offerType)->with('success', 'Successfully Order Closed');
    ...
    */
    ...
    public function ssg_items_edit(Request $request)
    {
        ...
        return view('sales/visitExceptional/editItems', compact('resultPro','pointid','retailderid','routeid','catid','resultCatDefault'));
    }
                               ...
                               $regular_free_id = DB::table('tbl_order_free_qty')->insertGetId(
                                ...
                                if($lastSlabSku->and_qty>0){
                                  ...
                                  DB::table('tbl_order_regular_and_free_qty')->insert(
                                    ...
                                    'total_free_value'      => $lastSlabSku->and_qty * $lastSlabSku->and_value,
                                    'created_by'            => Auth::user()->id,
                                    'ipaddress'             => request()->ip(),
                                    'hostname'              => $request->getHttpHost()
                                    ]
    ...
    */
    ...
    public function ssg_visit_process_submit(Request $request)
    {
        ...
        return Redirect::to('/visit-exception')->with('success', 'Successfully Visit Done.'); 
    }
    ...
    */
    ...
    public function ssg_nonvisit_process_submit(Request $request)
    {
        ...
        return Redirect::to('/visit-exception')->with('success', 'Successfully Non-visit Done.'); 
    ...
    */    
    ...
    public function ssg_order_manage()
    {
        ...
        return view('sales/visitExceptional/orderManage', compact('selectedMenu','pageTitle','resultOrderList'));
    }
    ...
    public function all_invoice_exception($orderId,$retailderid,$routeid)
    {
        ...
        $resultOrderList = DB::table('tbl_order_details')
                        ->select('tbl_order_details.*', DB::raw('SUM(tbl_order_details.order_qty) AS total_qty'),
                        DB::raw('SUM(tbl_order_details.p_grand_total) AS total_value'),'tbl_order.retailer_id','tbl_order.route_id','tbl_order.offer_type')
                        ->join('tbl_order', 'tbl_order_details.order_id', '=', 'tbl_order.order_id')
                        ->where('tbl_order_details.order_det_type', 'Confirmed')
                        ->where('tbl_order.global_company_id', Auth::user()->global_company_id)
                        ->where('tbl_order.order_id', $orderId)
                        ->where('tbl_order.fo_id', Auth::user()->id)
                        ->groupBy('tbl_order_details.partial_order_id')                    
                        ->orderBy('tbl_order.order_id','DESC')                    
                        ->get();  
       

        return view('sales/visitExceptional/visitManageNew', compact('selectedMenu','pageTitle','resultOrderList'));
    }
    ...
    public function ssg_order_edit_process($orderId,$retailderid,$routeid,$partial_order_id)
    {
        ...
        */
        ...
        return view('sales/visitExceptional/categoryWithOrderEdit', compact('selectedMenu','pageTitle','resultRetailer','resultCategory','retailderid','routeid','pointID','distributorID','resultCart','prevoiusBalanceCommission'));
    }
     ...
     public function ssg_add_to_edit_cart_products(Request $request)
    {
        ...
        $partial_order_id = $request->get('partial_id');
    ...
    */
    ...
    public function ssg_bucket_edit($orderid,$pointid,$routeid,$retailderid,$partial_id)
    {
        ...
        $resultCartPro  = DB::table('tbl_order_details')
                        ...
                        'tbl_order.retailer_id','tbl_order.order_status','tbl_order.order_count','tbl_order_details.order_det_status','tbl_order_details.partial_order_id')
                        ->join('tbl_product_category', 'tbl_product_category.id', '=', 'tbl_order_details.cat_id')
                        ->join('tbl_order', 'tbl_order.order_id', '=', 'tbl_order_details.order_id')
        ...
        return view('sales/visitExceptional/bucketEdit', compact('selectedMenu','pageTitle','pointid','retailderid','routeid','distributorID',
    ...
    public function ssg_order_valuewise_process($retailderid,$routeid,$orderid,$amount,$catid,$offerid,$pagestatus,$partialOrder,$offerType)
    {
        if($pagestatus==1)
        {
            $selectedMenu   = 'Visit'; 
        ...
        return view('sales/visitExceptional/categoryWithOrderValueWise', compact('selectedMenu','pageTitle','resultRetailer','resultCategory','retailderid','routeid','pointID','distributorID','resultCart','valueSum','pagestatus','prevoiusBalanceCommission'));
    }
    ...
    public function ssg_add_to_cart_value_wise_products(Request $request)
    {
        ...
        if($request->get('free_amount') >= $totalValue)
        {
            ...
            return Redirect::to('/order-process-valuewise-exception/'.$request->get('retailer_id').'/'.$request->get('route_id').'/'.$request->get('orderid').'/'.$lessAmount.'/'.$request->get('reference_catid').'/'.$request->get('reference_offerid').'/'.$request->get('reference_pagestatus').'/'.$request->get('partialOrder').'/'.$request->get('offerType'))->with('success', 'Successfully Added Add To Cart.');
        }
        else
        {
            return Redirect::to('/order-process-valuewise-exception/'.$request->get('retailer_id').'/'.$request->get('route_id').'/'.$request->get('orderid').'/'.$request->get('free_amount').'/'.$request->get('reference_catid').'/'.$request->get('reference_offerid').'/'.$request->get('reference_pagestatus').'/'.$request->get('partialOrder').'/'.$request->get('offerType'))->with('warning', 'Sorry, not match amount');
        }        
    }
    ...
    public function ssg_category_products_free(Request $request)
    {
        ...
        return view('sales/visitExceptional/allProductListFreeCommission', compact('resultProduct','categoryID','retailerID','offerTypeCategory'));
    }


    public function ssg_delete_order($orderID,$retailderid,$routeid,$partial_id)
    {

        //dd($orderID,$retailderid,$routeid,$partial_id);

        if($partial_id=='part_1')
        {
            ...
            return Redirect::to('/visit-exception/')->with('success', 'Successfully Deleted Order.');
        }
        else
        {
            $details = DB::table('tbl_order_details')
                    ->where('order_id', $orderID)
                    ->where('partial_order_id', $partial_id)
                    ->get();
            ...
            {
                $totalQty   = $totalQty + $value->order_qty;
                $totalPrice = $totalPrice + $value->p_total_price;
            }

            $final = DB::table('tbl_order')->where('order_id', $orderID)->first();

            $finalQty   = ($final->total_qty) - ($totalQty);
            $finalPrice = ($final->total_value) - ($totalPrice);

            //dd($finalQty,$totalPrice);

            DB::table('tbl_order')->where('order_id', $orderID)->update(
                [
                    'total_qty'         => $finalQty,
                    'total_value'       => $finalPrice,
                    'grand_total_value' => $finalPrice
                ]
            );
