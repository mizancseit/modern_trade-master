					...
					<table class="table table-bordered table-striped table-hover dataTable js-exportable">
                        ...
                        @if(sizeof($resultReqList) > 0)   
                            ...
                            <tr>
								...
								<th>
                                    <a href="{{ URL('/dist/free-req-details-list/'.$ReqRow->req_id) }}" title="Click To Details" target="_blank">
                                       {{ $ReqRow->req_no }}
                                    </a>
                                </th>
                             ...
                             @endphp
								...
								<th> 
                                    @php
                                    if($ReqRow->req_status == 'acknowledge'){
                                    @endphp
    									<a href="{{ URL('/dist/free-req-order-analysis/'.$ReqRow->req_id) }}" target="_blank" title="Click To Analysis">
                                            Analysis
                                        </a> || 
    									
    									<a href="{{ URL('/dist/free-req-canceled/'.$ReqRow->req_id) }}" target="_blank" title="Click To Cancel">
                                            Cancel
                                        </a>
                                    @php
                                    }elseif($ReqRow->req_status == 'canceled'){
                                        echo "Canceled";
                                    }
                                @endphp
								</th>			
