                                ...
                                <div class="col-sm-12">
                                   <label for="point">Point:*</label>
                                    <div class="form-group">
                                       ...
                                       @foreach($point as $pointok)
                                        <option value="{{$pointok->point_id}}">{{$pointok->point_name}}</option>
                                       @endforeach
                                        </select>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-hover dataTable js-basic-example dataTable">
              ...
              @if(sizeof($route) > 0)   
                    ...
                    @foreach($route as $routeSetup) 
                    <tr>
                         <th>{{$serial}}</th>
                         <th>{{$routeSetup->company_name }}</th>
                         <th>{{$routeSetup->point_name }}</th>
                        <th>{{$routeSetup->rname}}</th>
                        <th>{{$routeSetup->details}}</th>
                        <th><input type="button" name="route_edit" id="edit" value="Edit" class="btn bg-red btn-block btn-sm waves-effect" data-toggle="modal" onclick="editRoute('{{ $routeSetup->route_id }}')" style="width: 70px;"">
                        <input type="button" name="route_delete" id="delete" value="Delete" class="btn bg-red btn-block btn-sm waves-effect" data-toggle="modal" onclick="deleteRoute('{{ $routeSetup->route_id}}')" style="width: 70px; margin-top: 0px;"></th>
                    </tr>
