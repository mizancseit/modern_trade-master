...
class DistributorFreeRequisition extends Controller
{
	/**
	*
	...
	**/
    ...
    public function free_req_manage()
    {
        $selectedMenu    = 'Free Requisition';             // Required Variable
        $selectedSubMenu = 'Requisition';             // Required Variable
        $pageTitle       = 'Requisition';            // Page Slug Title

        $ParentList	= DB::select("SELECT * FROM dist_free_requisition WHERE req_id not in (
						SELECT dist.req_id FROM dist_free_requisition dist JOIN dist_free_req_details disDet ON dist.req_id = disDet.req_id)
		...
		if(sizeof($ParentList)>0)
		{
			...
			{
				DB::table('dist_free_requisition')
        			->where('req_id', $rowParentList->req_id)
        	        ->delete();
			}
		}

        $countReq = DB::table('dist_free_requisition')
		...
		$resultReqList = DB::select("SELECT u.display_name, p.point_name, dr.* 
										FROM dist_free_requisition dr JOIN users u ON dr.distributor_in_charge = u.id 
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList','countReq'));
    }
	
	
	public function free_req_add()
    {
        $selectedMenu    = 'Free Requisition';             // Required Variable
        $selectedSubMenu = 'Requisition';             // Required Variable
		...
		$LastReqId = DB::select("SELECT (MAX(req_id) + 1) as last_req_id FROM dist_free_requisition");						

        return view('sales/requisitionProcess/distributor/freeRequisition/req_add', compact('selectedMenu','selectedSubMenu','pageTitle','reqAddList','LastReqId'));
    }
	
	
	public function free_req_process(Request $req)
    {
		...
		if ($req->isMethod('post')) 
		{
			...
			$point=DB::insert('insert into dist_free_requisition(distributor_in_charge, point_id, req_no, req_by, 
			...
			return Redirect::to('/dist/free-req-list-product/'.$req_id)->with('success', 'Successfully Requisition Added.');
			   
		}
    }
	
	
	public function free_req_list_product($req_id)
	{
		$selectedMenu    = 'Free Requisition';              // Required Variable
		$selectedSubMenu = 'Requisition';             // Required Variable
		$pageTitle       = 'Requisition Manage';     // Page Slug Title
		...
		$resultCart     = DB::select("SELECT SUM(req_value) as grand_total_value FROM dist_free_req_details WHERE req_id = '".$req_id."'");


		 $lastReq = DB::table('dist_free_requisition')
		...
		if(sizeof($lastReq)>0){
         ...
         if(sizeof($lastReq)>0){
        	...
        	$resultProduct = DB::select("SELECT tbl_product.category_id,tbl_product_category.name as cname,tbl_product.name,tbl_product.depo as price,tbl_product.unit,e.product_id as id,sum(e.free_delivery_qty) AS qty FROM (
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_special_free_qty WHERE distributor_id = $distributor_id AND delivery_date>'".$date."' AND (status=0 OR status=3)
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_special_and_free_qty WHERE distributor_id = $distributor_id AND delivery_date>'".$date."' AND status=0
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_free_qty WHERE distributor_id = $distributor_id  AND delivery_date>'".$date."' AND status=0
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_regular_and_free_qty WHERE distributor_id = $distributor_id AND delivery_date>'".$date."' AND status=0
		) AS e 
		...
		group by e.product_id");
     	...
     	$resultProduct = DB::select("SELECT tbl_product.category_id,tbl_product_category.name as cname,tbl_product.name,tbl_product.depo as price,tbl_product.unit,e.product_id as id,sum(e.free_delivery_qty) AS qty FROM (
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_special_free_qty WHERE distributor_id = $distributor_id AND (status=0 OR status=3)
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_special_and_free_qty WHERE distributor_id = $distributor_id AND status=0
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_free_qty WHERE distributor_id = $distributor_id AND status=0
		UNION ALL
		SELECT order_date,point_id,distributor_id,product_id,free_delivery_qty FROM tbl_order_regular_and_free_qty WHERE distributor_id = $distributor_id AND status=0
		) AS e 
		INNER JOIN tbl_product ON tbl_product.id = e.product_id
		INNER JOIN tbl_product_category ON tbl_product_category.id = tbl_product.category_id
		group by e.product_id");
		...
		return view('sales/requisitionProcess/distributor/freeRequisition/categoryWithOrder', compact('selectedMenu','selectedSubMenu','pageTitle',
	...
	public function free_req_add_product(Request $request)
	{
        ...
        if($request->isMethod('post'))
        {
            ...
            for($m=0;$m<$countRows;$m++)
            {
                if($request->get('qty')[$m]!='')
                {
                    ...
                    DB::table('dist_free_req_details')->insert(
                        [
            ...
            return Redirect::to('/dist/free-req-manage/')->with('success', 'Successfully Added.');
        }
		
	}
	
	public function free_req_category_products(Request $request)
    {
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/allProductList', compact('resultProduct','categoryID','lastReq'));
    }
	
	
	
	public function free_req_bucket($reqid)
    {
		$selectedMenu    = 'Free Requisition';             // Required Variable
		$selectedSubMenu = 'Requisition';             // Required Variable
		$pageTitle       = 'Requisition Manage';            // Page Slug Title
		...
		$resultReqPro = DB::select("SELECT rdet.req_det_id as id, rdet.wastage_qty, u.display_name as distributor_in_charge, p.point_name, pc.name AS catname, 
								...
								FROM dist_free_requisition dr JOIN users u ON dr.distributor_in_charge = u.id 
								JOIN dist_free_req_details rdet ON rdet.req_id = dr.req_id
								JOIN tbl_point p ON p.point_id = dr.point_id 
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/bucket', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
    }
	
	
	public function free_req_send($req_id)
	{
		if($req_id)
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req_id)->update(
				[
					'req_status'	=> 'send',
		...
		return Redirect::to('/dist/free-req-send-list/')->with('success', 'Successfully Send.');
	}
	
	
	public function free_req_send_list()
	{
		$selectedMenu    = 'Free Requisition Send';             // Required Variable
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_send_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList'));			
	...
	public function free_req_pending_list()
	{
		$selectedMenu    = 'Free Requisition Pending';             // Required Variable
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_pending_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList'));			
	...
	public function free_req_acknowledge(Request $req)
	{
		if($req->isMethod('post'))
		{
			...
			foreach($req->input('reqid') as $rowReqId)
			{
				...
				if($ordackVal == 'YES')
				{
					...
					$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$rowReqId)->update(
						[
		...
		return Redirect::to('/dist/free-req-analysis-list/')->with('success', 'Successfully Acknowledge.');
	}
	
	
	public function free_req_analysis_list()
	{
		$selectedMenu    = 'Free Requisition Analysis';             // Required Variable
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_analysis_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function free_req_order_analysis($reqid)
	{
		...
		$isDepot = DB::select("SELECT u.display_name as distributor_in_charge, p.point_name,p.is_depot, pc.name AS catname, 
		...
		if(sizeof($isDepot)>0)
		{
			...
			{
				$resultReqPro = DB::select("SELECT u.display_name as distributor_in_charge, p.point_name, pc.name AS catname, 
								pr.name proname, rdet.wastage_qty, pr.stock_qty, pr.distri as price, rdet.req_det_id, rdet.req_qnty, rdet.req_value, dr.req_status
								FROM dist_free_requisition dr JOIN users u ON dr.distributor_in_charge = u.id 
								JOIN dist_free_req_details rdet ON rdet.req_id = dr.req_id
								JOIN tbl_point p ON p.point_id = dr.point_id 
								JOIN tbl_product_category pc ON pc.id = rdet.cat_id
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_order_analysis', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	}
	...
	public function free_req_approved(Request $req)
	{
		if($req->isMethod('post'))
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req->input('req_id'))->update(
						[
							'req_status'  => 'approved',
			...
			foreach($req->input('req_det_id') as $reqDetId)
			{
				...
				$DepotReqDetUpd = DB::update("UPDATE dist_free_req_details SET approved_qnty = $act_aprvd_qnty,
		...
		return Redirect::to('/dist/free-req-all-approved-list/')->with('success', 'Successfully Approved.');
	}
	...
	public function req_approved($req_id)
	{
		if($req_id)
		{
			...
			$DepotReqDetUpd = DB::update('UPDATE dist_free_req_details SET approved_qnty = req_qnty, approved_value = req_value 
										WHERE req_id = ?', [$req_id]);		
		}
	...
	*/
	...
	public function free_req_all_approved_list()
	{
		$selectedMenu    = 'Free Requisition Approved';             // Required Variable
        $selectedSubMenu = 'Requisition Approved';             // Required Variable
        $pageTitle       = 'Requisition Approved';            // Page Slug Title
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_all_approved_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
		
	}
	
	
	public function free_req_open_order_list($reqid)
    {
		$selectedMenu    = 'Home';             // Required Variable
		$selectedSubMenu = 'Home';             // Required Variable
		$pageTitle       = 'Requisition Details';            // Page Slug Title
		...
		$resultReqPro = DB::select("SELECT u.display_name as distributor_in_charge, p.point_name, pc.name AS catname, 
								pr.name proname, rdet.req_qnty, rdet.req_value, dr.req_status,rdet.approved_qnty,rdet.approved_value
								FROM dist_free_requisition dr JOIN users u ON dr.distributor_in_charge = u.id 
								JOIN dist_free_req_details rdet ON rdet.req_id = dr.req_id
								JOIN tbl_point p ON p.point_id = dr.point_id 
								JOIN tbl_product_category pc ON pc.id = rdet.cat_id
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_delivery_download_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function free_req_deliver($req_id)
	{
		if($req_id)
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req_id)->update(
			...
			$DepotReqDetUpd = DB::update('UPDATE dist_free_req_details SET delevered_qnty = approved_qnty, delevered_value = approved_value 
										WHERE req_id = ?', [$req_id]);				
		}
		
		return Redirect::to('/dist/free-req-all-approved-list/')->with('success', 'Successfully Delivered.');
	}
	
	public function free_req_delivered_list()
	{
		$selectedMenu    = 'Free Requisition Delivered';             // Required Variable
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_delivered_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function free_req_canceled($req_id)
	{
		if($req_id)
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req_id)->update(
		...
		return Redirect::to('/dist/free-req-analysis-list')->with('success', 'Successfully Canceled.');
	}
	
	
	public function free_req_canceled_list()
	{
	...
	public function req_receive($req_id)
	{
		if($req_id)
		{
			...
			$DepotReqDetUpd = DB::update('UPDATE dist_free_req_details SET received_qnty = delevered_qnty, received_value = delevered_value 
										WHERE req_id = ?', [$req_id]);	
	...
	*/
	...
	public function req_receive(Request $req)
	{
		if ($req->isMethod('post')) 
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req->input('reqid'))->update(
						[
							'req_status'  => 'received',
							'received_by'  => Auth::user()->id,
	...
	*/
	...
	//3rd version
	public function free_req_receive(Request $req)
	{
		if ($req->isMethod('post')) 
		{
			$DepotReqUpd = DB::table('dist_free_requisition')->where('req_id',$req->input('reqid'))->update(
			...
			foreach($req->input('req_det_id') as $req_det_id)
			{
				...
				$DepotReqDetUpd = DB::update("UPDATE dist_free_req_details SET received_qnty = '".$recvd_qnty."', 
																		   received_value = '".$recvd_value."'
										WHERE req_det_id = ?", [$req_det_id]);	
			
			}		 
			
			$isDepot = DB::table('dist_free_requisition')
						->join('tbl_point', 'tbl_point.point_id', '=', 'dist_free_requisition.point_id')
                        ->where('dist_free_requisition.req_id', $req->input('reqid'))
                        ->first();
		...
		return Redirect::to('/dist/free-req-delivered-list/')->with('success', 'Successfully Received.');
	}
	...
	public function free_req_delivered_details_list($reqid)
    {
		$selectedMenu    = 'Home';             // Required Variable
		$selectedSubMenu = 'Home';             // Required Variable
		$pageTitle       = 'Requisition Details';            // Page Slug Title
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_delivered_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function free_req_details($reqid)
    {
		$selectedMenu    = 'Free Requisition Send';             // Required Variable
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
        
		
    }

	public function free_req_details_list($reqid)
    {
		//$selectedMenu    = 'Free Requisition Send';             // Required Variable
		$selectedMenu    = 'Free Requisition Analysis';  
		$selectedSubMenu = 'Requisition Send';             // Required Variable
		$pageTitle       = 'Requisition Details';            // Page Slug Title
	...
	public function free_req_delivery_challan($reqid)
    {
		$selectedMenu    = 'Free Requisition Delivered';             // Required Variable
		$selectedSubMenu = 'Home';             // Required Variable
		$pageTitle       = 'Requisition Details';            // Page Slug Title
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_delivery_challan', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
        
		
    }
	
	
	public function free_req_delivery_received_list($reqid)
    {
		$selectedMenu    = 'Free Requisition Delivered';             // Required Variable
		$selectedSubMenu = 'Home';             // Required Variable
		$pageTitle       = 'Requisition Details';            // Page Slug Title
		...
		*/
        ...
        return view('sales/requisitionProcess/distributor/freeRequisition/req_delivery_receive_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	private function req_receive_stock($req_id)
	{
		...
		if($req_id)
		{
			...
			$resultReqData = DB::select("SELECT dr.*,  rdet.product_id, rdet.cat_id, rdet.received_qnty, rdet.received_value
								FROM dist_free_requisition dr JOIN dist_free_req_details rdet ON rdet.req_id = dr.req_id
								WHERE dr.req_id = '".$req_id."'
								AND dr.req_status = 'received'
								AND dr.is_active = 'YES'
								ORDER BY dr.req_id DESC");
	...
	public function export(Request $Request) 
	{
		...
		$items  = DB::table('dist_free_requisition')
		->select('users.display_name as Depot Name','tbl_point.point_name AS Point Name','tbl_product.companyid AS Company Code','tbl_product.sap_code AS Material Code','tbl_product_category.name AS Category','tbl_product.name AS Material Name','dist_free_req_details.req_qnty AS Qty','dist_free_req_details.req_value AS Value')
		->join('users', 'users.id', '=', 'dist_free_requisition.distributor_in_charge')
		->join('dist_free_req_details', 'dist_free_req_details.req_id', '=', 'dist_free_requisition.req_id')
		->join('tbl_point', 'tbl_point.point_id', '=', 'dist_free_requisition.point_id')
		->join('tbl_product_category', 'tbl_product_category.id', '=', 'dist_free_req_details.cat_id')
		->join('tbl_product', 'tbl_product.id', '=', 'dist_free_req_details.product_id')
		->where('dist_free_requisition.req_id', $reqID)
		->orderBy('dist_free_requisition.req_id','DESC')                    
		->get();
...
//$items = Item::all();
	...
	public function free_req_items_edit(Request $request)
    {
        
        $depotResult  = DB::table('dist_free_req_details')
                        ->select('dist_free_req_details.req_det_id','dist_free_req_details.wastage_qty','dist_free_req_details.product_id','dist_free_req_details.req_qnty','tbl_product.depo','tbl_product.name')
                        ->join('tbl_product', 'tbl_product.id', '=', 'dist_free_req_details.product_id')
                        ->where('dist_free_req_details.req_det_id', $request->get('id'))
                        ->first();


        return view('sales/requisitionProcess/distributor/freeRequisition/editReqItems', compact('depotResult'));
    }

    public function free_req_items_edit_submit(Request $request)
    {
        
        $price          = $request->get('items_qty') * $request->get('items_price');
        $price2         = $request->get('wastage_qty') * $request->get('items_price');

        DB::table('dist_free_req_details')->where('req_det_id',$request->get('id'))->update(
            [
    ...
    public function free_req_items_delete(Request $request)
    {
        $id = $request->get('id');

        $itemsDelete = DB::table('dist_free_req_details')->where('req_det_id',$id)->delete();
