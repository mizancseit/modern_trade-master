...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
			...
			function getProductQntityByProId($Pro_Id,$RecordSource)
			{
				foreach($RecordSource as $rowRecord)
				{
					if($Pro_Id == $rowRecord->product_id)
						return 	$rowRecord->order_qty;
					
				}
				
			}
			
			function getProductPriceByProId($Pro_Id,$RecordSource)
			{
				foreach($RecordSource as $rowRecord)
				{
					if($Pro_Id == $rowRecord->product_id)
						return 	$rowRecord->p_total_price;
			...
			function getWastageQntityByProId($Pro_Id,$RecordSource)
			{
				foreach($RecordSource as $rowRecord)
				{
					if($Pro_Id == $rowRecord->product_id)
						return 	$rowRecord->wastage_qty;
					
				}
				
			}
			
			function getStockQntityByProId($Pro_Id,$RecordSource)
			{
				foreach($RecordSource as $rowRecord)
				{
					if($Pro_Id == $rowRecord->product_id)
						return 	$rowRecord->stock_qty;
			...
			function getDemandQntityByProId($Pro_Id,$RecordSource)
			{
				foreach($RecordSource as $rowRecord)
				{
					if($Pro_Id == $rowRecord->product_id)
						return 	$rowRecord->demQnty;
                        ...
                        <tbody>
                            @if(sizeof($resultProduct) > 0)
                                ...
                                $serial = 1;
								 ...
								 $stockResult = DB::select("SELECT product_id,(sum(if(inventory_type=1,product_qty,0))- sum(if(inventory_type=2,product_qty,0))) as stock_qty FROM depot_inventory 
                                        WHERE point_id =$pointID group by product_id");
                             

								 $demandData  = DB::select("
											Select product_id, sum(tbl_order_details.order_qty) as demQnty from tbl_order_details
											JOIN tbl_order ON tbl_order_details.order_id = tbl_order.order_id
											WHERE tbl_order.point_id = $pointID AND tbl_order_details.order_det_type = 'Confirmed'
											GROUP BY tbl_order_details.product_id");
                                ...
                                <tr>
                                    ...
                                    <th>{{ $products->name }}</th>
                                     @if(getProductQntityByProId($products->id,$lastOrderData))
                                    <th><input type="number" class="form-control" id="qty{{$serial}}" name="qty[]" maxlength="8" pattern="[1-9]" min="1" value="{{ getProductQntityByProId($products->id,$lastOrderData) }}" style="width: 80px;" onkeyup="addQty({{$serial}})" onmouseout="totalsFO();" readonly="">
                                    </th>
                                    <th><input type="number" class="form-control" id="value{{$serial}}" name="value[]" maxlength="8" value="{{getProductPriceByProId($products->id,$lastOrderData)}}" style="width: 100px;" readonly=""></th>
                                    @else
                                    ...
                                    @endif
                                     @if(getWastageQntityByProId($products->id,$lastOrderData))
                                    <th><input type="number" class="form-control" id="wastageQty{{$serial}}" name="wastageQty[]" maxlength="8" value="{{getWastageQntityByProId($products->id,$lastOrderData)}}" style="width: 80px;" onkeyup="totalsWastagesFO();addWastage({{$serial}})"></th>
                                     @else 
                                     ...
                                     @endif  
                                     <th>{{ getStockQntityByProId($products->id,$stockResult) }} </th>
                                     <th>{{ getDemandQntityByProId($products->id,$demandData) }}</th>                                            
                                </tr>
                                @php
                                $serial ++;
                                @endphp
