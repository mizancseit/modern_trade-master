...
class BillingRequisition extends Controller
{
	/**
	*
	...
	**/
    ...
    public function req_manage()
    {
        ...
        return view('ModernSales::requisition/req_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList'));
    }
	...
	public function req_add()
    {
        ...
        return view('ModernSales::requisition/req_add', compact('selectedMenu','selectedSubMenu','pageTitle','reqAddList','LastReqId'));
    }
	...
	public function req_list_product($req_id)
	{
		...
		return view('ModernSales::requisition/categoryWithOrder', compact('selectedMenu','selectedSubMenu','pageTitle',
	...
	public function req_category_products(Request $request)
    {
        ...
        return view('ModernSales::requisition/allProductList', compact('resultProduct','categoryID'));
    }
	...
	public function req_bucket($reqid)
    {
        ...
        return view('ModernSales::requisition/bucket', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
    }
	...
	public function req_send_list()
	{
        ...
        return view('ModernSales::requisition/req_send_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList'));			
	...
	public function req_acknowledge_list()
	{
        ...
        return view('ModernSales::requisition/req_acknowledge_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_pending_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/req_pending_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList','divList',
	...
	public function req_analysis_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/req_analysis_list', compact('selectedMenu','approved','pageTitle','resultReqList','divList','custList'));			
	...
	public function req_analysis_list_new(Request $req)
	{
		...
		$divList = DB::table('tbl_division')->orderBy('div_id','asc')->get();
		$catList = DB::table('tbl_product_category')->orderBy('id','asc')->get();
		$prodList = DB::table('tbl_product')->orderBy('id','asc')->get();
		$custList = DB::table('users')
                        ->select('users.*')
						->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'users.id')
                        ->where('users.user_type_id', '5')
		...
		$resultAnalysisList = DB::select("SELECT drd.order_id, drd.order_det_id, pcat.id as cat_id, pcat.name as pro_cat_name, 
								...
								FROM mts_order dr JOIN mts_order_details drd ON dr.order_id = drd.order_id
								left JOIN users u ON u.id = dr.customer_id
								left JOIN tbl_product prd ON prd.id = drd.product_id 
								left JOIN tbl_product_category pcat ON pcat.id = drd.cat_id 
        ...
        return view('ModernSales::requisition/analysis/req_analysis_list', 
	...
	public function req_order_analysis($reqid)
	{
        ...
        return view('ModernSales::requisition/req_order_analysis', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	}
	...
	public function req_order_analysis_new($prodid)
	{
        ...
        return view('ModernSales::requisition/analysis/req_order_analysis', compact('selectedMenu','pageTitle','resultReqPro'));
	}
	...
	*/
	...
	public function req_approved_list()
	{
        ...
        return view('ModernSales::requisition/req_approved_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_all_approved_list()
	{
		...
		$resultReqList = DB::table('mts_order')
        ->select('mts_order.order_id','mts_order.order_no','mts_order.order_date','mts_order.order_status','mts_order.approved_by','mts_order.approved_date','mts_order.req_status','mts_customer_list.name','mts_customer_list.customer_id','mts_customer_list.address','mts_customer_list.route_id','mts_customer_list.sap_code','mts_party_list.party_id','mts_party_list.name as partyName')
        ->join('mts_party_list', 'mts_order.party_id', '=', 'mts_party_list.party_id')
        ...
        return view('ModernSales::requisition/req_all_approved_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_open_order_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_delivery_download_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_bill_details_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_bill_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_billed_process($req_id)
	{
		if($req_id)
		{
			$DepotReqUpd = DB::table('mts_order')->where('order_id',$req_id)->update(
						[
							'req_status'  => 'billed',
			...
			$DepotReqDetUpd = DB::update('UPDATE mts_order_details SET deliverey_qty = approved_qty, delivery_value = approved_value WHERE order_id = ?', [$req_id]);				
										
					
			$orderDetails = DB::table('mts_order_details') 
			        ->select('order_id','party_id', 'product_id', 'deliverey_qty', 'cat_id',DB::raw('SUM(delivery_value) AS delivery_value'),'entry_by')
			        ...
			        foreach ($orderDetails as $key => $orderDetail) {
						...
						$product_stock = [ 
							'out_qty' => $orderDetail->deliverey_qty,
							'stock_qty' => ($tblProductStock) ? ($tblProductStock->stock_qty - $orderDetail->deliverey_qty) : 0,  
						]; 
		...
		return Redirect::to('/modern-reqAllApprovedList/')->with('success', 'Successfully Billed.');
		//return Redirect::to('/modern-reqAllBilledList/')->with('success', 'Successfully Billed.');
	}
	public function readyForBilledBulk(Request $request)
	{
		
		foreach ($request->order as $req_id => $value) { 
			if($req_id)
			{
				$DepotReqUpd = DB::table('mts_order')->where('order_id',$req_id)->update(
							[
	...
	public function req_factory_wise_split($req_id)
	{
		if($req_id)
		{
			$reqAllFactory = DB::select("SELECT c.id as ori_factory_id, c.sap_code, c.name as ori_factory_name, c.short_code, 
										...
										JOIN tbl_product p ON rdet.product_id = p.id 
	...
	private function new_factory_details($req_id, $newReqId, $fact_code)
	{
		if($req_id && $newReqId && $fact_code)
		{
			...
			$reqFactItem = DB::select("SELECT  rdet.*  
										FROM depot_req_details rdet JOIN tbl_product p ON p.id = rdet.product_id
										JOIN tbl_company c ON c.sap_code = p.companyid
	...
	public function req_billed_list()
	{
        ...
        return view('ModernSales::requisition/req_billed_list', compact('selectedMenu','approved','pageTitle','resultBilledList'));			
	...
	public function req_all_billed_list()
	{
		...
		$resultReqList = DB::table('mts_order')
        ->select('mts_order.order_id','mts_order.order_no','mts_order.order_date','mts_order.order_status','mts_order.approved_by','mts_order.approved_date','mts_order.req_status','mts_order.is_download','mts_order.billed_by','mts_order.billed_date','mts_customer_list.name','mts_customer_list.customer_id','mts_customer_list.address','mts_customer_list.route_id','mts_customer_list.sap_code','mts_party_list.party_id','mts_party_list.name as partyName')
        ->join('mts_party_list', 'mts_order.party_id', '=', 'mts_party_list.party_id')
        ->join('mts_customer_list', 'mts_order.customer_id', '=', 'mts_customer_list.customer_id') 
        ->where('mts_order.req_status','billed')
        ...
        return view('ModernSales::requisition/req_all_billed_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_ready_for_delivery()
	{
        ...
        return view('ModernSales::requisition/factory.req_ready_delivery_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_delivery_item_list($reqid)
    {
        ...
        return view('ModernSales::requisition/factory.req_delivery_item_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_all_delivered_list()
	{
        ...
        return view('ModernSales::requisition/factory.req_all_delivered_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_canceled_list()
	{
        ...
        return view('ModernSales::requisition/req_canceled_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_all_canceled_list()
	{
        ...
        return view('ModernSales::requisition/req_all_canceled_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	*/
	...
	public function req_delivered_list()
	{
        ...
        return view('ModernSales::requisition/req_delivered_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_received_list()
	{
        ...
        return view('ModernSales::requisition/req_received_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_all_received_list()
	{
        ...
        return view('ModernSales::requisition/factory.req_all_received_list', compact('selectedMenu','approved','pageTitle','resultReqList'));			
	...
	public function req_approved_details_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_approved_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_delivered_details_list($reqid)
    {
        ...
        return view('ModernSales::requisition/factory.req_delivered_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_received_details_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_received_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_details_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_details_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_delivery_challan($reqid)
    {
        ...
        return view('ModernSales::requisition/req_delivery_challan', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function req_delivery_received_list($reqid)
    {
        ...
        return view('ModernSales::requisition/req_delivery_receive_list', compact('selectedMenu','pageTitle','resultReqPro','reqid'));
	...
	public function sales_order_export(Request $Request) 
	{
		...
		->select('users.display_name as Depot Name','tbl_point.point_name AS Point Name','tbl_product.companyid AS Company Code',
		'tbl_product.sap_code AS Material Code','tbl_product_category.name AS Category','tbl_product.name AS Material Name',
		...
		->join('tbl_product_category', 'tbl_product_category.id', '=', 'depot_req_details.cat_id')
		->join('tbl_product', 'tbl_product.id', '=', 'depot_req_details.product_id')
...
//$items = Item::all();
    	...
    	if($request->hasFile('imported-file'))
    	{
    		...
    		if(!empty($data) && $data->count()){
    			...
    			if(!empty($insert)){
					...
					DB::select("UPDATE tbl_product JOIN tbl_sap_stock ON tbl_sap_stock.material_no = tbl_product.sap_code
					SET tbl_product.stock_qty = tbl_sap_stock.stock_qty WHERE tbl_sap_stock.is_active=1");
	...
	public function depot_items_edit(Request $request)
    {
        ...
        $depotResult  = DB::table('depot_req_details')
                        ->select('depot_req_details.req_det_id','depot_req_details.product_id','depot_req_details.req_qnty','tbl_product.depo','tbl_product.name')
                        ->join('tbl_product', 'tbl_product.id', '=', 'depot_req_details.product_id')
                        ->where('depot_req_details.req_det_id', $request->get('id'))
                        ->first();


        return view('ModernSales::requisition/editReqItems', compact('depotResult'));
    }
	...
	public function req_active_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/active_inactive/req_active_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList','divList'));			
	...
	public function req_inactive_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/active_inactive/req_in_active_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultReqList','divList'));			
	...
	public function cust_active_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/active_inactive/cust_active_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultCustList','divList'));			
	...
	public function cust_inactive_list(Request $req)
	{
        ...
        return view('ModernSales::requisition/active_inactive/cust_in_active_list', compact('selectedMenu','selectedSubMenu','pageTitle','resultCustList','divList'));			
	...
	public function export(Request $Request) 
	{
		...
		$items  = DB::table('depot_requisition')
		->select('users.display_name as Depot Name','depot_requisition.req_no as req_no','users.sap_code as sap_code','tbl_point.point_name AS Point Name','tbl_product.companyid AS CompanyCode',
		'tbl_product.sap_code AS MaterialCode','tbl_product_category.name AS Category','tbl_product.name AS Material Name',
		'depot_req_details.req_qnty AS Qty','depot_req_details.req_value AS Value',
...
','Ship To Party','Ref PO No','Ref PO Date','req del date','Order Note','Debit/Credit Note','Order Reason','Currency','Material Code','Qty','Plant Code','Storage Loc','Batch','Item Category','ZASV','ZEXP','ZFRH','ZTHP','ZSCR','ZRMS','ZAST');
	...
	public function export_sale_order(Request $Request) 
	{
		...
		$items  = DB::table('mts_order')
		->select('mts_order.order_id', 'mts_order.po_no','mts_order.order_no as order_no','mts_order.auto_order_no','mts_party_list.party_id as party_id',
			'mts_party_list.name as partyName', 'mts_customer_list.sap_code as sap_code', 'mts_customer_list.name AS customerName','tbl_product.companyid AS CompanyCode', 
			'tbl_product.sap_code AS MaterialCode', 'tbl_product.mrp', 'tbl_product_category.name AS Category','tbl_product_category.g_code AS g_code', 'tbl_product.name AS Material Name', 
			'mts_order_details.order_qty AS Qty', 'mts_order_details.p_unit_price as sq_price','mts_order_details.order_total_value AS Value', 'mts_order_details.cat_id', 
			'mts_order_details.approved_qty AS AppQty','mts_order_details.approved_value AS AppValue')
		...
		// dd($items); 
		$custom_array[] = array('Customer ID.','Material Code', 'Order qty', 'Mrp Price', 'SQ Price','Total Value', 'Order Number Or Automation SO Number','PO No');

		$s = 1;
		foreach ($items as $key => $value) 
		{			
				$commission  = DB::table('mts_categroy_wise_commission')
				->where('order_id', $value->order_id) 
				->where('cat_id', $value->cat_id)
				->first(); 

				 
				// New query by mazid 2-23-2021
				$g_code = $value->g_code+1;
				 
				// New query by mazid 2-23-2024
				$g_code = $value->g_code+1; 
				//$result = $value->mrp - ($value->mrp * ($commission->commission / 100));
				$salePrice = ($commission) ? ( $value->mrp - ($value->mrp * ($commission->commission / 100)) ) : $value->mrp ;  // =F2-(F2*C2*100)
				$totalValueWithCommission = $value->Qty * $value->sq_price;
				// $totalValue = $totalValueWithCommission - $salePrice;
				$totalValue = $salePrice * $value->Qty;
				// $SQPrice = $totalValue/$value->Qty;
				$custom_array[] = array(
					'Customer ID.' 		 => $value->sap_code,
					'Material Code'		 => $value->MaterialCode, 
					'Order qty' 		 => $value->Qty, 					 					
					'Mrp Price' 		 => number_format((float)$value->mrp , 2, '.', ''), 					
					'SQ Price' 		 	 => number_format((float)$salePrice, 2, '.', ''), 					
					'Total Value' 	 	 => number_format((float)$totalValue, 2, '.', ''), 					
					'Order Number Or Automation SO Number' => $value->order_no,
					'PO No ' => $value->po_no
				);

				$totalQty = DB::table('mts_order_details')
				...
				$ReqDownUpd = DB::table('mts_order')->where('order_id',$value->order_id)->update(
						[
	...
	public function export_sale_order_Old(Request $Request) 
	{

		$items  = DB::table('mts_order')
		->select('mts_order.order_id','mts_order.order_no as order_no','mts_order.auto_order_no','mts_party_list.party_id as party_id','mts_party_list.name as partyName', 'mts_customer_list.sap_code as sap_code', 'mts_customer_list.name AS customerName','tbl_product.companyid AS CompanyCode', 'tbl_product.sap_code AS MaterialCode', 'tbl_product_category.name AS Category','tbl_product_category.g_code AS g_code', 'tbl_product.name AS Material Name', 'mts_order_details.order_qty AS Qty', 'mts_order_details.order_total_value AS Value', 'mts_order_details.approved_qty AS AppQty','mts_order_details.approved_value AS AppValue')
		->join('mts_party_list', 'mts_party_list.party_id', '=', 'mts_order.party_id')
		->join('mts_order_details', 'mts_order_details.order_id', '=', 'mts_order.order_id')
		->join('mts_customer_list', 'mts_customer_list.customer_id', '=', 'mts_order.customer_id')
		->join('tbl_product_category', 'tbl_product_category.id', '=', 'mts_order_details.cat_id')
		...
		$custom_array_old[] = array('SO Ref No','Company Code','SO Type','Dis.Cha','Sold To Party','Ship To Party','Ref PO No','Ref PO Date','req del date','Order Note','Debit/Credit Note','Order Reason','Currency','Material Code','Qty','Plant Code','Storage Loc','Batch','Item Category','ZASV','ZEXP','ZFRH','ZTHP','ZSCR','ZRMS','ZAST');
		$custom_array[] = array('Ref No.','Purch.Group(EKGRP)','Receiving plant', 'Vendor Acc.(LIFNR)','Material Code','Order qty','STOP FOR CUSTOMER','Mail id');
		...
		foreach ($items as $value) 
		{
				...
				$g_code = $value->g_code+1;
				$custom_array[] = array(
					'Ref No.' 			 => $value->order_id.$value->g_code,
					'Purch.Group(EKGRP)' => 'FGS',
					'Receiving plant'    => '1613',
					'Vendor Acc.(LIFNR)' => 'v'.$g_code,
					'Material Code'		 => $value->MaterialCode,
					'Order qty' 		 => $value->Qty,
					'STOP FOR CUSTOMER'  => $value->auto_order_no.'-'.$value->sap_code.'-'.$value->partyName.'-'.$value->g_code,
					'Mail id' 			 => '',
				);
