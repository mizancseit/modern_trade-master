            ...
            <div class="card">
                ...
                <div class="body">                    
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <button type="button" class="btn bg-pink btn-block btn-lg waves-effect" onclick="allReturnOrdersM()">Search</button>
                        </div>
                    </div>                                  
                </div>
            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                @if(sizeof($resultOrderList) > 0)   
                                    ...
                                    @php
                                    $totalQty  += $orders->total_return_only_qty;
                                    $totalValue += $orders->total_return_only_value;
                                    @endphp                    
                                    <tr>
                                        <th>{{ $serial }}</th>
                                        <th>
                                            <a href="{{ URL('/fo/change-only-category-products/'.$orders->return_only_order_id) }}" title="Confirm Delivery" target="_blank">
                                                {{ $orders->return_only_order_no }}
                                            </a>
                                        </th>
                                        <th>{{ $orders->return_only_order_date }}</th>
                                        <th>{{ $orders->first_name }}</th>                        
                                        <th>{{ $orders->name }} <br /> {{ $orders->mobile }}</th>                        
                                        <th style="text-align: right;">{{ $orders->total_return_only_qty }}</th>                        
                                        <th style="text-align: right;">{{ number_format($orders->total_return_only_value,2) }}</th> 
                                        {{-- <th>
                                            <a href="{{ URL('/invoice/'.$orders->return_only_order_id.'/'.$orders->fo_id) }}" title="Invoice Details" target="_blank">
