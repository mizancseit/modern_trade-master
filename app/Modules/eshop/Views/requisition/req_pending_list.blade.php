    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
				...
				<form action="{{ URL('/reqPendingList') }}" method="get">
				    ...
				    <div class="row">	
						...
						<div class="col-sm-2">
                           <select class="form-control show-tick" id="req_status" name="req_status">
								...
								<option value="acknowledge" <?=($sel_req_status == 'acknowledge')?'selected':''; ?>>ACK</option>
								<option value="send" <?=($sel_req_status == 'send')?'selected':''; ?>>NOT ACK</option>
							</select>
						</div>
				...
				<form action="{{ URL('/reqAcknowledge') }}" method="POST">	
				 {{ csrf_field() }}
					...
					<table class="table table-bordered table-striped table-hover dataTable js-exportable">
                        ...
                        @if(sizeof($resultReqList) > 0)   
							...
							$grand_depo_ava_bal = 0; 
							$grand_credit_limit = 0;
							$grand_total_debited = 0;
							...
							$depot_bal = DB::select("SELECT credit_limit, distributor_id 
								FROM tbl_distributor_balance WHERE distributor_id 
							...
							if(sizeof($depot_bal)>0)
							{
							  ...
							  $distributor_id = $depot_bal[0]->distributor_id; 
							}		
							...
							*/
							...
							$SqlPartyBalance = "SELECT actual_closing_balance 
							FROM tbl_party_ledger WHERE party_id = '".$distributor_id."' 
							ORDER BY ledger_id DESC LIMIT 1";	
								
							$PartyBalance = DB::select("$SqlPartyBalance");
							
							if(sizeof($PartyBalance)>0)
							{
								$depo_ava_bal = $PartyBalance[0]->actual_closing_balance;
							}
							
							
							$total_debited = $credit_limit - $depo_ava_bal;
                            ...
                            <tr>
								...
								<th>
                                    <a href="{{ URL('/reqDetails/'.$ReqRow->req_id) }}" title="Click To Details" target="_blank">
                                       {{ $ReqRow->display_name . ' &nbsp;' . '('. $ReqRow->sap_code .')' }}
                                    </a>
                                </th>
						...
						<?php							
							$totalQty += $ReqRow->totQnty;
                            $totalValue += $ReqRow->totVal;
							
							$grand_depo_ava_bal += $depo_ava_bal; 
							$grand_credit_limit += $credit_limit;
							$grand_total_debited += $total_debited;
						?>	
                            ...
                            @endforeach
							 ...
							 <tr>
								...
								<th>{{ $totalQty }}</th>
								<th>{{ $totalValue }}</th>
								<th>{{ $grand_depo_ava_bal }}</th>
								<th>{{ $grand_credit_limit }}</th>
								<th>{{ $grand_total_debited }}</th>
								<th></th>
                            </tr>
