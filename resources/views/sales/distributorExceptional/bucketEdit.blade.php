        ...
        <div class="container-fluid">
            <div class="block-header">
                <div class="row">
                    ...
                    <div class="col-sm-3">
                         <a href="{{ URL('/close-distributor-order-exceptional/'.$resultInvoice->order_id.'/'.Request::segment(4)) }}"> 
            ...
            <form action="{{ URL('/order-edit-submit-exceptional') }}" name="form" id="form" method="POST">
                {{ csrf_field() }}    <!-- token -->
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                               ...
                                               <th style="text-align: right;">
                                                    <input type="number" name="qty[]" id="qty{{ $serial }}" value="@if($itemsPro->delivered_qty>= -1){{ $itemsPro->delivered_qty }}@else{{ $itemsPro->order_qty }}@endif" class="form-control" style="width: 100px;" onkeyup="qty({{ $serial }},{{ $itemsPro->order_qty }})" min="1" autocomplete="off">
                                                </th>
                                               ...
                                               <th style="text-align: right;">@if($itemsPro->wastage_value=='') {{ 0 }} @else {{ $itemsPro->wastage_value }} @endif</th>
                                                <th style="text-align: right;">
                                            ...
                                            @endif
                                                ...
                                                @if($items->partial_order_id =='part_'.$resultInvoice->order_count && $items->order_det_status=='Closed' )

                                                @php                                             
                                                    $reultRegularGift  = DB::table('tbl_order_free_qty AS fq')
                                                    ->select('fq.type','fq.status','fq.order_id','fq.auto_order_no','fq.catid','fq.product_id','fq.total_free_qty','fq.free_value','tbl_product.id','tbl_product.name AS proname','tbl_product.mrp','tbl_product.depo','fq.free_id')
                                                ...
                                                ->select('fq.order_id','fq.catid','fq.product_id','fq.total_free_qty','fq.free_value','fq.special_id','tbl_product.id','tbl_product.name AS proname','tbl_product.mrp','tbl_product.depo')
                                                ->join('tbl_product', 'tbl_product.id', '=', 'fq.product_id')
                                                // ->where('fq.status','0')      
                                                ->where('fq.order_id',$resultInvoice->order_id)
                                                ...
                                                @php                                             
                                                    ...
                                                    ->select('fq.order_id','fq.catid','fq.product_id','fq.total_free_qty','fq.free_value','fq.free_delivery_qty','fq.free_id','tbl_product.id','tbl_product.name AS proname','tbl_product.mrp','tbl_product.depo')
                                                    ->join('tbl_product', 'tbl_product.id', '=', 'fq.product_id')
                                                    ->where('fq.status','0')      
                                                    ->where('fq.order_id',$resultInvoice->order_id)
                                                    ->get();
                                                ...
                                                ->select('fq.free_id','fq.order_id','fq.catid','fq.product_id','fq.total_free_qty','fq.free_value','fq.free_delivery_qty','fq.special_id','tbl_product.id','tbl_product.name AS proname','tbl_product.mrp','tbl_product.depo')
                                                ->join('tbl_product', 'tbl_product.id', '=', 'fq.product_id')
                                                // ->where('fq.status','0')      
                                                ->where('fq.order_id',$resultInvoice->order_id)
                                                ->where('fq.special_id', $itemsPro->free_id)
                                                ...
                                                @endphp
                                                    ...
                                                    <th style="text-align: right;">
                                                     ...
                                                     <input type="hidden" name="free_product_id[]" id="free_product_id{{ $serial }}" value="{{ $itemsPro->product_id }}" >
                                                     <input type="hidden" name="free_value[]" id="free_value{{ $serial }}" value="{{ $itemsPro->depo }}" >
                                                    <input type="number" name="freeqty[]" id="freeqty{{ $serial }}" value="@if($itemsPro->free_delivery_qty==''){{$itemsPro->total_free_qty}}@else{{$itemsPro->free_delivery_qty}}@endif" class="form-control" style="width: 70px;" onkeyup="freeqty({{ $serial }})" min="1" autocomplete="off">
                                                    </th>
                                                    <th style="text-align: right;">
                                                        <input type="hidden" name="total_free_value[]" id="total_free_value{{ $serial }}" value="{{ $itemsPro->free_value }}" >{{ $itemsPro->free_value }}
                                                    ...
                                                    <input type="number" name="andFreeQty[]" id="andFreeQty{{ $serial }}" value="@if($reultSpecialAnd->total_free_qty!=''){{$reultSpecialAnd->total_free_qty}}@else{{$reultSpecialAnd->free_delivery_qty}}@endif" class="form-control" style="width: 70px;" onkeyup="freeqty({{ $serial }})" min="1" autocomplete="off">
                                                    </th>
                                                   <th style="text-align: right;">
                                                        <input type="hidden" name="total_and_free_value[]" id="total_and_free_value{{ $serial }}" value="{{ $reultSpecialAnd->free_value }}" >
                                                        {{ $reultSpecialAnd->free_value }}</th>
                                            ...
                                            @if(sizeof($orderTypePartial)>0 && $orderTypePartial->order_det_status=='Closed')
                                                ...
                                                <tr>
                                                    ...
                                                    <th colspan="6">{{ round($commissionAmount - $valueSum).' ('.$svwRows->commission.'%'.') ' }} 
                                                        <a href="{{ URL('/order-process-valuewise/'.$resultInvoice->retailer_id.'/'.$resultInvoice->point_id.'/'.$svwRows->order_id.'/'.$totalFreeValueWiseCommission.'/'.$items->catid.'/'.$svwRows->offer_id.'/'.'3'.'/'.Request::segment(4)) }}"> &nbsp;&nbsp;
                                    ...
                                    <div class="row" style="text-align: center;">
                                        ...
                                        @if(sizeof($orderTypePartial)>0 && $orderTypePartial->order_det_status=='Closed')
                                            ...
                                            @else
                                                ...
                                                @if(sizeof($resultBundleOffers)>0)
                                                    <div class="col-sm-1">
                                                        <input name="offer_type" type="radio" id="radio_7{{ $resultBundleOffers->iId }}" class="radio-col-red" value="{{ $resultBundleOffers->iId }}" data-toggle="modal"  @if(sizeof($resultBundleOffersGift)>0) checked="" @endif onclick="showBundleProduct('{{ $resultBundleOffers->iId }}','2')">
                                                        <label for="radio_7{{ $resultBundleOffers->iId }}"> Bundle</label>  
                                        ...
                                        @if($orderTypePartial->order_det_status=='Closed') 
                                        ...
                                        <div class="col-sm-3">
                                            ...
                                            @elseif($checkExclusiveAvailableItems==Null && $checkExclusiveAdded==Null)
                                                <button onclick="statusProcessM(1)" id="confirmDelivery" type="button" class="btn bg-pink btn-block btn-lg waves-effect">Confirm Delivery</button>
                                            @elseif($resultBundleAvailableItems >0)
                                                @if($resultBundleAddedItems>0)
                                                    <button onclick="statusProcessM(1)" id="confirmDelivery" type="button" class="btn bg-pink btn-block btn-lg waves-effect">Confirm Delivery</button>
                                                @else
                                                    <button onclick="statusProcessM(1)" id="confirmDelivery" type="button" class="btn bg-pink btn-block btn-lg waves-effect"  disabled="">Confirm Delivery</button>
                                                @endif
                                            @elseif($resultBundleAvailableItems==0)
                                                <button onclick="statusProcessM(1)" id="confirmDelivery" type="button" class="btn bg-pink btn-block btn-lg waves-effect">Confirm Delivery</button>
                                            @else
