...
class ChangeController extends Controller
{
	...
	**/
    ...
    {		
		...
		{		
			...
			$resRetChangData = DB::table('tbl_return')
								->select('tbl_return.return_order_date','tbl_return.point_id', 'tbl_return.distributor_id', 'tbl_return.fo_id', 'tbl_return.route_id', 'tbl_return.retailer_id', 
								...
								->join('tbl_retailer', 'tbl_retailer.retailer_id', '=', 'tbl_return.retailer_id')
								->leftJoin('tbl_product as p1', 'p1.id', '=', 'tbl_return_details.return_product_id')
								->leftJoin('tbl_product as p2', 'p2.id', '=', 'tbl_return_details.change_product_id')
								->where('tbl_return.return_order_id', $return_order_id)
								->get();

			$resultCartPro  = DB::table('tbl_return_details')
                        ->select('tbl_return_details.return_product_id','tbl_return_details.return_cat_id','tbl_return_details.return_value','tbl_return_details.return_qty','tbl_return_details.change_cat_id','tbl_return_details.change_product_id','tbl_return_details.change_value','tbl_return_details.change_qty','tbl_return_details.return_order_id','tbl_product_category.id AS catid','tbl_product_category.name AS catname','tbl_return.return_order_id','tbl_return.distributor_id','tbl_return.fo_id','tbl_return.return_order_type','tbl_return.return_order_no','tbl_return.retailer_id','tbl_return.global_company_id','tbl_product.id AS pid','tbl_product.name AS proname')
			...
			$resultInvoice  = DB::table('tbl_return')->select('tbl_user_details.first_name','tbl_user_details.cell_phone','tbl_return.return_auto_order_no','tbl_return.chalan_no','tbl_return.chalan_date','tbl_return.update_date','tbl_return.global_company_id','tbl_return.return_order_id','tbl_return.return_order_type','tbl_return.return_order_id','tbl_return.distributor_id','tbl_return.fo_id','tbl_return.return_order_type','tbl_return.return_order_no','tbl_return.retailer_id','tbl_return.global_company_id','tbl_return.entry_date','tbl_retailer.name','tbl_retailer.mobile')

                        ->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'tbl_return.distributor_id')
                        ->join('tbl_retailer', 'tbl_retailer.retailer_id', '=', 'tbl_return.retailer_id')
        ...
        $resultDistributorInfo = DB::table('tbl_return')->select('tbl_user_details.first_name','tbl_user_details.cell_phone','tbl_return.global_company_id','tbl_return.return_order_id','tbl_return.return_order_type','tbl_return.distributor_id','tbl_return.fo_id','tbl_point.point_id','tbl_point.point_name','tbl_route.route_id','tbl_route.rname')

                        ->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'tbl_return.distributor_id')
                        ->join('tbl_point', 'tbl_point.point_id', '=', 'tbl_return.point_id')
                        ->join('tbl_route', 'tbl_route.route_id', '=', 'tbl_return.route_id')
                        ->where('tbl_return.global_company_id', Auth::user()->global_company_id)                        
                        //->where('tbl_return.distributor_id',Auth::user()->id)                        
                        ->where('tbl_return.return_order_id',$return_order_id)
                        ->first();
        ...
        $resultFoInfo  = DB::table('tbl_return')->select('tbl_user_details.first_name','tbl_user_details.cell_phone','tbl_return.distributor_id','tbl_return.fo_id')
                        ->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'tbl_return.fo_id')
                        ->where('tbl_return.global_company_id', Auth::user()->global_company_id)
                        //->where('tbl_return.distributor_id',Auth::user()->id)                        
                        ->where('tbl_return.return_order_id',$return_order_id)
			...
			if(sizeof($resRetChangData) >0 )
			{
				...
				$returnDate = $resRetChangData[0]->return_order_date;
			...
			$currentDate = date('m');
			$for = date('m',strtotime($returnDate)) ;
			
			if($currentDate==$for)
			{
				//echo 'Confirm';
				$layout = 'sales/change/categoryWiseReturnChange';
			}
			else
			{				
				//echo 'Expired';
				$layout = 'sales/change/categoryWiseReturnChangeReport';
			}
							
		    return view($layout,compact('selectedMenu','selectedSubMenu','pageTitle', 'resRetChangData','resultCategory','return_order_id','pointID','distributorID','routeid','retailderid','foID','tbl_return_exception','returnDate','resultCartPro','resultInvoice','resultDistributorInfo','resultFoInfo'));					
    ...
    {
    	//print_r($request->get('change_cat_id'));
    	//exit();
		...
		{	
			...
			$autoAdd  = $lastOrderId;
            $chalanNO = 'RCC-'.Auth::user()->sap_code.'-'.date('ymd').$autoAdd;
            //dd($chalanNO);
			...
			//echo count($request->get('change_product_id'));
			//exit();
			...
			$returnRows = count($request->get('return_product_id'));
			for($m=0;$m<$returnRows;$m++)
			{
				if($request->get('change_qty')[$m]=='')
				{
			...
			if(sizeof($resRetChangData) >0 )
			{
				if($resRetChangData[0]->total_approved_value > $resRetChangData[0]->total_return_value)
				{
					...
					$retailer_info['retailer_invoice_no'] = $resRetChangData[0]->return_order_no;
    ...
    public function ssg_return_order_report()
    {
        $selectedMenu   = 'return-change-report';             // Required Variable
        $pageTitle      = 'return-change-report';            // Page Slug Title
        ...
        if(session('userType')!='FO')
        {
        	$resultFO   = DB::table('tbl_return')
                        ->select('tbl_return.global_company_id','tbl_return.return_order_type','tbl_return.return_order_id','tbl_return.fo_id','tbl_user_details.user_id','tbl_user_details.first_name','tbl_user_details.middle_name','tbl_user_details.last_name')
                        ->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'tbl_return.fo_id')                       
                        ->where('tbl_return.return_order_type', 'Delivered')
                        ->where('tbl_return.distributor_id', Auth::user()->id)
                        ->where('tbl_return.global_company_id', Auth::user()->global_company_id)
                        ->groupBy('tbl_return.fo_id')
                        ->orderBy('tbl_return.return_order_id','DESC')                    
        ...
        if(session('userType')!='FO')
        {
        	$resultOrderList = DB::table('tbl_return')
                        ...
                        ->where('tbl_return.global_company_id', Auth::user()->global_company_id)
                        ->where('tbl_return.distributor_id', Auth::user()->id)
                        ->whereBetween(DB::raw("(DATE_FORMAT(tbl_return.update_date,'%Y-%m-%d'))"), array($todate, $todate))
                        ->orderBy('tbl_return.return_order_id','DESC')                    
                        ->get();
        ...
        return view('sales/change/returnOrderReport', compact('selectedMenu','pageTitle','resultFO','resultOrderList'));
    }

    
	public function ssg_return_order_report_list(Request $request)
    {
        $fromdate   = date('Y-m-d', strtotime($request->get('fromdate')));
        $todate     = date('Y-m-d', strtotime($request->get('todate')));
        ...
        if($fromdate!='' && $todate!='' && $request->get('fos')=='')
        {
        	if(session('userType')!='FO')
        	{
	            $resultOrderList = DB::table('tbl_return')
	                        ...
	                        ->where('tbl_return.distributor_id', Auth::user()->id)
	                        ->where('tbl_return.global_company_id', Auth::user()->global_company_id)
	                        ->whereBetween(DB::raw("(DATE_FORMAT(tbl_return.update_date,'%Y-%m-%d'))"), array($fromdate, $todate))
	                        ->orderBy('tbl_return.return_order_id','DESC')                    
	                        ->get();
	        }
	        else
        ...
        return view('sales/change/returnOrdersListReport', compact('resultOrderList'));
    }

    public function return_change_products_report($return_order_id)
    {		
		$selectedMenu   = 'return-change-report';      // Required Variable
		$selectedSubMenu = 'Return Change Report';      // Required Variable
        $pageTitle      = 'return-change-report';        // Page Slug Title
        ...
        $resultDistributorInfo = DB::table('tbl_return')->select('users.display_name','tbl_user_details.first_name','tbl_user_details.cell_phone','tbl_return.global_company_id','tbl_return.return_order_id','tbl_return.return_order_type','tbl_return.distributor_id','tbl_return.fo_id','tbl_point.point_id','tbl_point.business_type_id','tbl_point.point_name','tbl_route.route_id','tbl_route.rname')
        ...
        $resultFoInfo  = DB::table('tbl_return')->select('tbl_user_details.first_name','tbl_user_details.cell_phone','tbl_return.distributor_id','tbl_return.fo_id')
		    ...
		    return view($layout,compact('selectedMenu','selectedSubMenu','pageTitle','resultCartPro','resultInvoice','orderMainId','foMainId','resultFoInfo','resultDistributorInfo'));					
